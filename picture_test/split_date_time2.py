...
last_h = 0
last_dta = ""
f = open("NODEDQ/NODEDQ_zd.csv",'rb')
rows = csv.reader(f)
rows.next()
for row in rows:
    data = row[1].split(" ")[0]
    ...
    if int(h) == last_h and data == last_dta:
        continue
    date_time = [row[1].split(":")[0],row[1].split(":")[1],row[1].split(":")[2]]
    fh = ':'
    date_timeh = fh.join(date_time)
    tt = (float(row[0]),date_timeh)
    words.append(tt)
    last_dta = data
    last_h = int(h)
f.close()

f = open("NODEDQ/zd2.csv","wb")
write = csv.writer(f)
write.writerow(["Turbidity","datetime"])
total = 0
for key in words:
    write.writerow(key)
    total += 1
