...
score_train_test=score_train_test.drop(['num'],axis=1)
...
card_t['consumesum1'] = t.groupby(['id'])['amount'].sum()
card_t['consumeavg1'] = t.groupby(['id'])['amount'].mean()
# card_t['consumemax1'] = t.groupby(['id'])['amount'].max()
card_t['consumemedian1'] = t.groupby(['id'])['amount'].median()
card_t.rename(columns={'amount':'cz_count1'},inplace=True)
...
# how_name=['shitang','qita','tushuguan','kaishui','chaoshi','xiyifang']
...
#         t1=t[t.is_weekend==i]
#         card_t = pd.DataFrame(t1.groupby(['id'])['amount'].count()).reset_index()
#         # print card_t
#         card_t['consumesum_'+how_name[i]] = t1.groupby(['id'])['amount'].sum()
#         # card_t['consumesumcha_'+how_name[i]] = (t.groupby(['id'])['amount'].sum())/card_t['consumesum_'+how_name[i]].mean()
#         card_t['consumeavg_'+how_name[i]] = t1.groupby(['id'])['amount'].mean()
#         # card_t['consumesumavg_'+how_name[i]] = t.groupby(['id'])['amount'].mean()/t['amount'].mean()
#         card_t['consumemax_'+how_name[i]] = t1.groupby(['id'])['amount'].max()
#         # card_t['consumemaxavg_'+how_name[i]] = t.groupby(['id'])['amount'].max()/card_t['consumemax_'+how_name[i]].mean()
#         card_t['consumemedian_'+how_name[i]] = t1.groupby(['id'])['amount'].median()
...
#         card=pd.merge(card,card_t,on=['id'],how='left')
#####################################################################
how_name=['qita','tushuguan','kaishui','jiaowuchu','wenyin','lingyu','chaoshi','shitang','xiyifang','xiaoche','xiaoyiyuan']
for i in range(len(how_name)):
    t=card_train_test[['id','how','time','amount']]
    t=t[t.how==how_name[i]]
    t['day_of_week'] = t.time.astype('str').apply(lambda x:date(int(x[0:4]),int(x[5:7]),int(x[8:10])).weekday()+1)
...
# train_test.to_csv('../input/train_test.csv',index=False)
###########################################################
dorm_train = pd.read_table('../train/dorm_train.txt',sep=',',header=-1)
dorm_train.columns = ['id','time','how']
dorm_train_test = dorm_train
dorm_train_test=dorm_train_test.drop_duplicates()
dorm_train_test['timestr']= dorm_train_test.time.astype('str').apply(lambda x:x[0:4]+x[5:7]+x[8:10])
dorm_out=dorm_train_test[dorm_train_test.how==1]
dorm = pd.DataFrame(dorm_out.groupby(['id','timestr'])['how'].count()).reset_index()
print dorm
dorm['out_mean']=dorm_out.groupby(['id'])['how'].mean()
dorm['out_meidan']=dorm_out.groupby(['id'])['how'].median()

dorm_in=dorm_train_test[dorm_train_test.how==0]
t = pd.DataFrame(dorm_in.groupby(['id','timestr'])['how'].count()).reset_index()
t['in_mean']=dorm_in.groupby(['id'])['how'].mean()
t['in_meidan']=dorm_in.groupby(['id'])['how'].median()
dorm=pd.merge(dorm,t,on=['id'],how='left')
